...
from logger import log_info, log_success, log_warning, log_error, log_title, log_debug
...
class User_Interface:
    ...
    async def _handle_bot_status(self, title: str | None, allow_upgrade: bool) -> None:
        if 'bot:play' not in await self.api.get_token_scopes(self.config.token):
            log_error('Your token is missing the bot:play scope. This is mandatory to use BotLi.\n'
        ...
        log_warning('BotLi can only be used by BOT accounts!\n')

        if not sys.stdin.isatty() and not allow_upgrade:
            log_warning('Start BotLi with the "--upgrade" flag if you are sure you want to upgrade this account.\n'
                        'WARNING: This is irreversible. The account will only be able to play as a BOT.')
            sys.exit(1)
        elif sys.stdin.isatty():
            log_warning('This will upgrade your account to a BOT account.\n'
                        'WARNING: This is irreversible. The account will only be able to play as a BOT.')
            approval = input('Do you want to continue? [y/N]: ')

            if approval.lower() not in ['y', 'yes']:
                log_info('Upgrade aborted.')
                sys.exit()
    ...
    async def _test_engines(self) -> None:
        for engine_name, engine_config in self.config.engines.items():
            log_info(f'Testing engine "{engine_name}" ... ')
    ...
    def _challenge(self, command: list[str]) -> None:
        ...
        log_success(f'Challenge against {challenge_request.opponent_username} added to the queue.')

    def _clear(self) -> None:
        self.game_manager.challenge_requests.clear()
        log_info('Challenge queue cleared.')

    def _create(self, command: list[str]) -> None:
        if len(command) < 3 or len(command) > 6:
            log_warning(COMMANDS['create'])
            return
        try:
            count = int(command[1])
        ...
        log_success(f'Challenges for {count} game pairs against {opponent_username} added to the queue.')

    async def _join(self, command: list[str]) -> None:
        if len(command) < 2 or len(command) > 3:
            log_warning(COMMANDS['join'])
            return
        password = command[2] if len(command) > 2 else None
        if await self.api.join_team(command[1], password):
            log_success(f'Joined team "{command[1]}" successfully.')

    def _leave(self, command: list[str]) -> None:
        if len(command) != 2:
            log_warning(COMMANDS['leave'])
            return
    ...
    async def _quit(self) -> None:
        ...
        log_warning('Terminating program ...')
    ...
    def _rechallenge(self) -> None:
        ...
        if last_challenge_event is None:
            log_warning('No last challenge available.')
            return
        if last_challenge_event['speed'] == 'correspondence':
            log_warning('Correspondence is not supported by BotLi.')
            return
    ...
    def _reset(self, command: list[str]) -> None:
        ...
        log_info('Matchmaking has been reset.')

    def _stop(self) -> None:
        if self.game_manager.stop_matchmaking():
            log_info('Stopping matchmaking ...')
        else:
            log_warning('Matchmaking isn\'t currently running ...')

    def _tournament(self, command: list[str]) -> None:
        if len(command) < 2 or len(command) > 4:
            log_warning(COMMANDS['tournament'])
            return
    ...
    def _whitelist(self, command: list[str]) -> None:
        if len(command) != 2:
            log_warning(COMMANDS['whitelist'])
            return
        self.config.whitelist.append(command[1].lower())
        log_success(f'Added {command[1]} to the whitelist.')

    def _help(self) -> None:
        log_info('These commands are supported by BotLi:\n')
        for key, value in COMMANDS.items():
            log_info(f'{key:11}\t\t# {value}')
